...
public class StubHttpConsumer {
    ...
    public void start(List<ContractEventDetails> broadcastContractEvents) {
        ...
        wireMockServer.addStubMapping(
                post(urlPathEqualTo("/consumer/block-event"))
                        .willReturn(aResponse().withStatus(responseStatus.value()))
                        .build());

        wireMockServer.addStubMapping(
                post(urlPathEqualTo("/consumer/contract-event"))
                        .willReturn(aResponse().withStatus(responseStatus.value()))
                        .build());

        wireMockServer.addStubMapping(
                post(urlPathEqualTo("/consumer/transaction-event"))
                        .willReturn(aResponse().withStatus(responseStatus.value()))
                        .build());

        wireMockServer.addStubMapping(
                post(urlPathEqualTo("/consumer/message-event"))
                        .willReturn(aResponse().withStatus(responseStatus.value()))
                        .build());
        ...
        wireMockServer.addMockServiceRequestListener(
                (request, response) -> {
                    if (request.getUrl().contains("/consumer/contract-event")) {
                        ...
                        try {
                            broadcastContractEvents.add(
                                    objectMapper.readValue(body, ContractEventDetails.class));
                        } catch (IOException e) {
                            e.printStackTrace();
                        }
                    }
