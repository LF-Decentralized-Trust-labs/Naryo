...
/**
 * A message interface to be broadcast from the Naryo application.
 *
...
@JsonSubTypes({
    ...
    @JsonSubTypes.Type(
            value = ContractEventFilterAdded.class,
            name = ContractEventFilterAdded.TYPE),
    @JsonSubTypes.Type(
            value = ContractEventFilterRemoved.class,
            name = ContractEventFilterRemoved.TYPE),
    @JsonSubTypes.Type(value = TransactionMonitorAdded.class, name = TransactionMonitorAdded.TYPE),
    @JsonSubTypes.Type(
            value = TransactionMonitorRemoved.class,
            name = TransactionMonitorRemoved.TYPE)
})
@JsonInclude(JsonInclude.Include.NON_NULL)
public interface NaryoMessage<T> {
    String getId();
