...
import io.naryo.chain.service.domain.io.*;
import io.naryo.chain.service.domain.wrapper.HederaBlock;
import io.naryo.chain.service.domain.wrapper.HederaTransactionReceipt;
...
import io.naryo.model.FilterSubscription;
...
import io.naryo.utils.AtomicBigInteger;
...
public class HederaService implements BlockchainService {
    ...
    public void processBlock(HederaBlock block) {
        ...
        try {
            ...
            List<ContractResultResponse> responsesFiltered =
                    this.filterAndGetContractResults(responses);
    ...
    public BlockResponse getBlock(BigInteger blockNumber) throws IOException, NotFoundException {
        ...
        BlocksResponse blocksResponse =
                this.newCall(request, new TypeReference<BlocksResponse>() {});
        ...
        HttpUrl.Builder httpBuilder =
                HttpUrl.parse(
                                nodeUrl
                                        + API_VERSION_PATH
                                        + LOGS_PATH.replace(
                                                CONTRACT_VAR, eventFilter.getContractAddress()))
                        .newBuilder();

        httpBuilder.addQueryParameter(ORDER, ASC);
        httpBuilder.addQueryParameter(LIMIT, nodeLimitPerRequest);
        httpBuilder.addQueryParameter(
                TIMESTAMP, GTE + ":" + startBlockInfo.getTimestamp().getFrom());
    ...
    /**
     * Register a contract event listener for the specified event filter, that gets triggered when
     * an event matching the filter is emitted within the Ethereum network.
     *
     * @param eventFilter The contract event filter that should be matched.
     ...
     * @return The block for the specified hash or nothing if a block with the specified hash does
     *     not exist.
     */
    @Override
